...
/* [Exportacao] */
part = "organizer"; // [organizer,plate,assembly] Parte, job ou verificacao de duas unidades.
...
assert(print_w <= 210 && overall_d <= 210 && overall_h <= 220,
...
echo(modular_bbox_mm=[print_w,overall_d,overall_h], connector_clearance_per_side=slide_clearance,
     connector_engagement_h=connector_h, assembly_vertical_course=assembly_vertical_course,
     coupled_body_pitch=overall_w+connector_depth+slide_clearance);
...
module female_connector(yc) {
  difference() {
    translate([-receiver_depth, yc-receiver_w/2, floor_t])
      cube([receiver_depth, receiver_w, side_wall_h]);
    ...
    translate([-connector_depth-slide_clearance, yc, floor_t-0.1])
      linear_extrude(height=overall_h-floor_t+1.2)
        male_profile(slide_clearance);
  }
}

module receiver_wall_pockets() {
  // O receptor vizinho avanca 1.5 mm sobre a longarina direita. Somente as
  // duas paredes laterais do receptor ocupariam material: estes rasgos as
  // recebem, mantendo o miolo da longarina para ancorar o macho.
  pocket_d = connector_receiver_wall + 0.1;
  cavity_half_w = connector_head_w/2 + slide_clearance;
  for (yc=connector_y, sy=[-1,1]) {
    y0 = sy < 0 ? yc-receiver_w/2-0.1 : yc+cavity_half_w;
    translate([overall_w-pocket_d, y0, floor_t])
      cube([pocket_d+0.1, connector_receiver_wall+0.2, side_wall_h+0.1]);
  }
}
...
module organizer_core() {
  difference() {
    ...
    if (modular_connectors) receiver_wall_pockets();
  }
}
...
module assembly() {
  organizer();
  translate([overall_w+connector_depth+slide_clearance,0,0]) organizer();
}
