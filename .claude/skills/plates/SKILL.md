---
name: plates
description: Monta UM 3MF de projeto do Bambu Studio com várias plates dentro, em vez de um arquivo por chapa. Usar quando um projeto tiver muitos jobs de impressão e o usuário quiser abrir uma vez só e imprimir plate por plate.
---

# 3MF multi-plate (projeto do Bambu Studio)

O 3MF que o OpenSCAD exporta só sabe de UMA chapa: um projeto com 15 jobs
vira 15 arquivos pra abrir na mão. O Bambu Studio tem um formato de PROJETO
que guarda várias plates no mesmo arquivo — é o que este script escreve.

```
python3 .claude/skills/plates/plates.py <saida.3mf> \
...
2. `--usable` é a área útil (170 por padrão, a margem de brim do CLAUDE.md).
...
3. Salve em `3mf/` (é job de impressão de verdade, o contrato do CLAUDE.md
   vale). Nome sugerido: `<modelo>-todas-as-plates.3mf`.
4. **Confirme no Bambu Studio** depois de gerar: as plates têm que aparecer
...
02.07.01 pra A1 mini). Duas peças:
...
- `3D/3dmodel.model` — os objetos e um `<item>` por peça, com a translação
  ...
  (180 → 216mm). A peça fica centrada em `origem_da_plate + (90, 90)`.
...
Os dois mecanismos são redundantes de propósito: mesmo que o Bambu ignore o
`model_settings.config`, a posição sozinha já joga cada peça na plate certa.

**Não escrevemos `Metadata/project_settings.config`.** Sem ele o Bambu Studio
...
se cai dentro de `[origem, origem+180]` da plate que o config declara, com
